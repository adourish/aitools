...
async def process_new_comprehensive():
    ...
    try:
        ...
        # Events containing these keywords are always skipped (noise/clutter)
        skip_keywords = [
            'onedrive', 'one drive', 'sharepoint', 'microsoft teams',
            'sign up', 'signup', 'sign-up',
            'registration', 'enroll', 'enrollment',
        ]
        ...
        for ei, event in enumerate(all_events_list):
            ...
            # Skip OneDrive updates, sign-ups, and other low-value event types
            if any(kw in summary_lower for kw in skip_keywords):
                logger.info(f"   Skipped (filtered keyword): {summary[:60]}")
                continue
