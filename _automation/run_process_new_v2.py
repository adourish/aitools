...
async def process_new_comprehensive():
    ...
    try:
        ...
        # Sign-up / registration events are always skipped — informational only, not action items
        signup_skip_patterns = ['sign up', 'signup', 'sign-up', 'registration', 'tryout', 'try out']
        ...
        for ei, event in enumerate(all_events_list):
            ...
            # Skip sign-up / registration events — informational, not action items
            if any(pat in summary_lower for pat in signup_skip_patterns):
