...
async def process_new_comprehensive():
    ...
    # Step 7: Save comprehensive output to local directory (Google Drive path)
    ...
    try:
        ...
        def build_task(item, priority_level):
            ...
            # Skip non-actionable items (including OneDrive update actions)
            ...
            if any(phrase in action_lower for phrase in (
                'informational', 'already handled', 'no action', 'fyi only', 'deadline passed',
                'onedrive', 'one drive', 'update onedrive', 'sync onedrive',
                'sign up', 'signup', 'registration confirmed', 'successfully registered',
            )):
                logger.info(f"   ⏭️  Skipped non-actionable: {action[:60]}")
                return None
