...
async def process_new_comprehensive():
    ...
    # Step 7.55: Filter out informational-only, sign-up, and expired threads
    logger.info("\n\U0001f9f9 STEP 7.55: Filtering out informational-only, sign-up, and expired threads...")
    actionable_analyses = []
    ...
    for analysis in thread_analyses:
        ...
        # Check for sign-up / registration / newsletter emails
        subject_lower = analysis.get('thread_subject', '').lower()
        signup_keywords = [
            'sign up', 'signup', 'sign-up', 'registration complete',
            'subscription confirmed', 'confirm your email', 'verify your email',
            'welcome to', 'account created', 'unsubscribe', 'email confirmation',
            'confirm your account', 'account confirmation', 'you\'re registered',
            'you have registered', 'registration confirmation',
        ]
        is_signup_email = any(kw in subject_lower for kw in signup_keywords)
        ...
        if is_signup_email:
            signup_count += 1
            logger.info(f"   Filtered (sign-up/registration): {analysis['thread_subject'][:60]}")
        elif is_informational and not analysis.get('follow_up_needed'):
            informational_count += 1
            logger.info(f"   Filtered (informational): {analysis['thread_subject'][:60]}")
        else:
            actionable_analyses.append(analysis)

    logger.info(f"   Kept {len(actionable_analyses)} actionable, filtered {informational_count} informational, {signup_count} sign-up, {expired_count} expired")
